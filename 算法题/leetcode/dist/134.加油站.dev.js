 ...
 * 5.循环结束后存在符合条件的就返回该点在原数组中的下标，否则返回-1
 */
var canCompleteCircuit = function canCompleteCircuit(gas, cost) {
  var gascost = [];
  ...
  var shifttime = gas.length;
  var result;

  while (shifttime) {
    var allgas = 0;

    for (var j = 0; j < gascost.length; j = j + 2) {
    ...
    var tempgas = gascost.shift();
    var tempcost = gascost.shift();
