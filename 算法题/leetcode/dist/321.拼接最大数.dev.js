...
// 4.拿到最后的max 即为返回值
var findyourBest = function findyourBest(arr, k) {
  var curmax = '';
  ...
  var maxvalue = Math.max.apply(Math, _toConsumableArray(arr));
...
var getCurMax = function getCurMax(firstmax, secondmax, k) {
  var firstindex = 0,
      secondindex = 0;
  var cur = [];

  for (var resultindex = 0; resultindex < k; resultindex++) {
    //一号数组遍历结束
    if (firstindex == firstmax.length) {
      cur.push.apply(cur, _toConsumableArray(secondmax.slice(secondindex, secondmax.length)));
      break;
    } //二号数组遍历结束


    if (secondindex == secondmax.length) {
      cur.push.apply(cur, _toConsumableArray(firstmax.slice(firstindex, firstmax.length)));
      break;
    }
...
var maxNumber = function maxNumber(nums1, nums2, k) {
  if (nums1.length == 0) {
  ...
  for (var i = 1, j = k - i; i < k; i++, j = k - i) {
    if (nums1.length < i || nums2.length < j) {
      continue;
    }

    var firstmax = findyourBest(nums1, i).split('');
    var secondmax = findyourBest(nums2, j).split('');
    var cur = getCurMax(firstmax, secondmax, k);
...
console.log(maxNumber([2, 5, 6, 4, 4, 0], [7, 3, 8, 0, 6, 5, 7, 6, 2], 15)); //理解错题目意思了 浪费了很多时间 但是思考的过程没有浪费晚上根据题目意思再思考一下~