...
// 抄答案！！！
var findRotateSteps = function findRotateSteps(ring, key) {
  var indexMap = {};
  ...
  var dfs = function dfs(ringI, keyI) {
    if (keyI == key.length) {
      return false;
    }
    ...
    try {
      for (var _iterator = indexMap[cur][Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var targetI = _step.value;
        var d1 = Math.abs(ringI - targetI);
        var d2 = ring.length - d1;
        var curMin = Math.min(d1, d2);
